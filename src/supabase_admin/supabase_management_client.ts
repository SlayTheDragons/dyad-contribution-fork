...
async function getGlobalSupabaseCli(): Promise<SupabaseCliLocation | undefined> {
  ...
  try {
    ...
    // Ignore lookup failures here. We'll surface a consolidated warning if we
    // exhaust all retries without finding the CLI.
    void error; // no-op to satisfy lint rules without logging every attempt.
  }
...
async function waitForSupabaseCli(appPath: string): Promise<SupabaseCliLocation> {
  ...
  logger.warn(
    "Supabase CLI not found after multiple checks. Skipping Supabase commands until it is installed.",
  );
